...
interface Contact {
  ...
  OpenedAt: string;
  'Company': string;
  'Comapany Revenue': string | number;
  'Description': string;
  'Sector': string;
  'Why Salesforce ?': string;
  // Additional email tracking fields
  [key: `Send Status ${number}`]: string;
  [key: `Time ${number}`]: string;
  [key: `Opened ${number}`]: string;
  [key: `OpenedAt ${number}`]: string;
...
// Component to render email history for a contact
const EmailHistory = ({ contact }: { contact: Contact }) => {
  const history = [];
  
  // Get all email history entries (2-6)
  for (let i = 2; i <= 6; i++) {
    const statusKey = `Send Status ${i}` as const;
    const timeKey = `Time ${i}` as const;
    const openedKey = `Opened ${i}` as const;
    const openedAtKey = `OpenedAt ${i}` as const;
    
    if (contact[statusKey] || contact[timeKey] || contact[openedKey] || contact[openedAtKey]) {
      history.push({
        id: i,
        status: contact[statusKey],
        time: contact[timeKey],
        opened: contact[openedKey],
        openedAt: contact[openedAtKey]
      });
    }
  }
  ...
  return (
    <div className="mt-2 ml-12 p-3 bg-gray-50 rounded-md">
      <h4 className="font-medium text-sm text-gray-700 mb-2">Email History:</h4>
      <div className="space-y-2">
        {history.map((item) => (
          <div key={item.id} className="text-xs text-gray-600 border-b border-gray-200 pb-2 last:border-0 last:pb-0">
            <div className="grid grid-cols-3 gap-2">
              <div>
                <span className="font-medium">Email {item.id}:</span>
              </div>
              <div>
                <span className="text-gray-500">Sent:</span> {item.time || 'N/A'}
              </div>
              <div>
                <span className="text-gray-500">Opened:</span> {item.opened === 'Yes' ? (
                  <span className="text-green-600">Yes ({new Date(item.openedAt).toLocaleString()})</span>
                ) : 'No'}
              </div>
            </div>
          </div>
        ...
        {contact['Schedule Date'] && (
          <div className="mt-2 pt-2 border-t border-gray-200 text-xs">
            <span className="font-medium">Next Scheduled:</span> {new Date(contact['Schedule Date']).toLocaleDateString()}
          </div>
        )}
      </div>
    </div>
...
const LeadCompaniesTable = () => {
  ...
  const toggleContactSelection = (rowNumber: number) => {
    setSelectedContacts(prev =>
      prev.includes(rowNumber)
        ? prev.filter(id => id !== rowNumber)
        : [...prev, rowNumber]
    );
  };
  ...
  const toggleSelectAll = () => {
    if (selectedCompany) {
      if (selectedContacts.length === selectedCompany.length) {
        setSelectedContacts([]);
      } else {
        setSelectedContacts(selectedCompany.map(contact => contact.row_number));
      }
    }
  };
                  ...
                  <div className="flex items-center space-x-3">
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        const selectedEmails = selectedCompany
                          ?.filter(contact => selectedContacts.includes(contact.row_number))
                          .map(contact => ({
                            email: contact.Email
                          })) || [];
                        ...
                        if (selectedEmails.length === 0) {
                          toast({
                            ...
                            description: 'Please select at least one contact to send the email to.',
                            variant: 'destructive',
                          });
                          return;
                        }
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <Table>
                      <TableHeader className="bg-gray-50">
                        <TableRow>
                          <TableHead className="w-[50px] font-medium text-gray-700">
                            <Checkbox
                              checked={selectedCompany && selectedContacts.length === selectedCompany.length && selectedCompany.length > 0}
                          ...
                          <TableHead className="w-[180px] font-medium text-gray-700">Opened At</TableHead>
                          <TableHead className="w-[50px]"></TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {selectedCompany.map((contact, index) => {
                          const isExpanded = expandedRows.includes(contact.row_number);
                          const hasHistory = Array.from({ length: 5 }, (_, i) => i + 2).some(i => 
                            contact[`Send Status ${i}` as keyof Contact] || 
                            contact[`Time ${i}` as keyof Contact]
                          ) || contact['Schedule Date'];
                          
                          return (
                            <>
                              <TableRow key={index} className={`hover:bg-gray-50 ${isExpanded ? 'bg-blue-50' : ''}`}>
                                <TableCell className="font-medium">
                                  <Checkbox
                                    checked={selectedContacts.includes(contact.row_number)}
                                    onCheckedChange={() => toggleContactSelection(contact.row_number)}
                                    disabled={!!contact['Send Status']}
                                    className={`h-4 w-4 rounded ${contact['Send Status'] ? 'opacity-50 cursor-not-allowed' : ''}`}
                                  />
                                </TableCell>
                                <TableCell className="font-medium">
                                ...
                                <TableCell className="text-sm text-gray-600">
                                  {contact['OpenedAt'] ? new Date(contact['OpenedAt']).toLocaleString() : 'N/A'}
                                </TableCell>
                                <TableCell className="text-right">
                                  {hasHistory && (
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        toggleRowExpand(contact.row_number);
                                      }}
